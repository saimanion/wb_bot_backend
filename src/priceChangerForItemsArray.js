...
async function priceChangerForItemsArray(itemsToUpdate){
    // Создания массивов с ценами которые нужно поставить и теми которые были изначально
    let newPricesArray = itemsToUpdate.map(item => ({
        nmID: item.id,
        price: parseInt(item.price)
    }));
    // console.log('newPricesArray', newPricesArray);

    let prevPricesArray = itemsToUpdate.map(item => ({
        nmID: item.id,
        price: parseInt(item.prevPrice)
    }));
    // console.log("prevPricesArray",prevPricesArray);
...
// Таймер на две минуты
    ...
    await new Promise(resolve => setTimeout(resolve, 120000 )).then(() => {
...
export default priceChangerForItemsArray
